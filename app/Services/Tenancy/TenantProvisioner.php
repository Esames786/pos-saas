...
class TenantProvisioner
{
    ...
    protected function seedTenantBaseData(Tenant $tenant, string $ownerPassword): void
    {
        ...
        $tenantPermissions = [
            ...
            'tenant.department-counts.index',
            'tenant.department-counts.create',
            'tenant.department-counts.store',
            'tenant.department-counts.show',
            'tenant.department-counts.edit',
            'tenant.department-counts.update',
            'tenant.department-counts.submit',
            'tenant.department-counts.approve',
            ...
            // BRANCH-DEVICE-PAIRING-1 (+HARDEN-1) — pairing-code lifecycle + device revocation
            // + the always-reachable security-management view. Owner-only; generation
            // additionally requires entitlement + rollout at request time, while the security
            // view + cancel + revoke work regardless (security controls).
            'tenant.offline-edge.pairing-code.generate',
            'tenant.offline-edge.pairing-code.cancel',
            'tenant.offline-edge.devices.revoke',
            'tenant.offline-edge.security',
            ...
            // Finished Goods CRUD (MANUF-6)
            'tenant.manufacturing.finished-goods.index',
            'tenant.manufacturing.finished-goods.create',
            'tenant.manufacturing.finished-goods.store',
            'tenant.manufacturing.finished-goods.show',
            'tenant.manufacturing.finished-goods.edit',
            'tenant.manufacturing.finished-goods.update',
            'tenant.manufacturing.finished-goods.destroy',
            // Scrap / Hard Waste CRUD (MANUF-7)
            'tenant.manufacturing.scrap.index',
            'tenant.manufacturing.scrap.create',
            'tenant.manufacturing.scrap.store',
            'tenant.manufacturing.scrap.show',
