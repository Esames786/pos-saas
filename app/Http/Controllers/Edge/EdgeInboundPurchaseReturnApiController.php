...
use App\Services\Edge\EdgeInboundPurchaseReturnIngestionService;
use App\Services\Tenancy\TenancyManager;
use Illuminate\Http\JsonResponse;
use Illuminate\Http\Request;

/** OFFLINE EDGE — F3: the thin device-authenticated ingestion endpoint for Edge purchase-return events (Cloud-only). */
class EdgeInboundPurchaseReturnApiController extends Controller
{
    public function __construct(private readonly EdgeInboundPurchaseReturnIngestionService $ingestion, private readonly TenancyManager $tenancy)
    {
    }
    ...
    public function store(Request $request): JsonResponse
    {
        ...
        if ((string) ($envelope['device_public_uuid'] ?? '') !== (string) $device->public_uuid) {
            return response()->json(['status' => 'refused', 'failure_code' => 'DEVICE_MISMATCH', 'sale_uuid' => $uuid, 'event_uuid' => $uuid, 'content_hash' => (string) ($envelope['content_hash'] ?? ''), 'message' => 'the envelope device does not match the authenticated device'], 403);
        }
