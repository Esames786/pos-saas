...
| 🟡 | `production_ready` `released` | dastawez aur kitchen sheet, magar **stock phir bhi nahi** | haan, ijazat ke saath |
...
Aur 🟡 zone, alag ijazat ke saath:

```
released   →  confirmed     (sirf jab us release par koi material issue NA hua ho)
```

`production_ready → confirmed` bhi isi guard par.
...
## 8. Owner se poochhe jaane wale sawal

1. **Cancelled booking jo paisa daba kar baithi hai** — kya us ki koi list honi
   chahiye? Aaj koi nahi chahta us ko.
2. **Reopen kis ko?** Sirf Owner, ya Manager ko bhi?
3. **`released → confirmed`** kya bilkul chahiye, ya sirf `confirmed` tak wapsi
   kaafi hai?
4. **Un-cancel ke baad** wohi purana status theek hai, ya operator khud chune?
