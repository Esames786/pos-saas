...
- **BILL-PREVIEW-WRONG-PRINT-1 / BILL-PREVIEW-UNHIDE-1 / TABLE-WORKSPACE-WIDTH-1 / TABLE-BILL-PREVIEW-PARITY-1 (canonical 7cd5924 → 636ab9d, 14 Sep 2026; live on production at acf33a8):** Online's shared bill-preview modal used to send the CART order to the network printer while showing a TABLE's bill; the fix stamps the modal with its mode + held sale ids, re-shows the card's Bill Preview, widens the Table Workspace on wide screens, and renders the table bill through the real receipt document (`tenant/printing/documents/receipt.blade.php`, new `@isset($tableBill)` block listing rounds + previously-paid rounds). **Edge assessment (19 Sep, not merged):** the Edge cashier has no shared preview modal and no "Send to network" on a bill preview — `previewBill` is a zero-mutation JSON totals call and every print goes through a print-job id (`/edge/local/pos/print-jobs/{job}/…`), so the wrong-target bug class does not exist on Edge. The receipt-template block is dormant on Edge (only `RestaurantTableSessionController::renderTableBillReceipt` passes `tableBill`; Edge renders via `PrintDocumentController::preview`), so Edge receipt output is byte-identical whether or not the delta is merged. Remaining drift: Online's table bill preview now shows per-round numbers/totals and a "Previously paid" list; Edge's Preview Bill shows the running totals only → **UX_DRIFT (presentation only, non-financial)** — for the next parity tranche, not for P5 certification.
