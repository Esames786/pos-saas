...
FULL_OFFLINE_PARITY         = 25 rows        ONLINE_REQUIRED = 4 (new-customer creation offline*, card payment/refund, QR email, Cloud admin)
FINANCIAL_PARITY_PENDING    = 0 rows in the matrix (Purchase Returns offline = FINANCIAL_PARITY_PENDING, Purchasing is outside the branch-POS matrix)
F1 sales returns / cash refunds: FULL (11 Sep 2026) · F2 supplier finance: FULL (12 Sep 2026)
* not inherent — next Edge build item (till-created customers need a Cloud ingestion contract)

NORMAL_OPERATOR_POS_PARITY_PERCENT = 25 / 28 = 89%   (all rows a branch operator runs: 25 FULL + card + QR email + new-customer; Cloud admin excluded)
FULL_OFFLINE_PARITY_PERCENT        = 25 / 29 = 86%   (every row in the matrix)
```
