...
| F3 | (next) **EDGE-RUNTIME-BOUNDARY-1** — restricted build/route-allowlist skeleton (no cloud admin/finance/purchasing/mfg/billing surface; no selling) + in parallel lock offline-numbering (§8), UUID-identity (§9), stock+split-brain (§10/§10b) → then EDGE-LOCAL-RUNTIME-1 → EDGE-LOCAL-AUTH-1 → EDGE-IDENTITY-1 → EDGE-LOCAL-POS-1 → EDGE-LOCAL-PRINT-1 → EDGE-SPLITBRAIN-STOCK-1 → EDGE-CONFIG-REFRESH-1 → OFFLINE-SYNC-ENGINE-1 → OFFLINE-RECOVERY-HARDEN-1 → EDGE-ENTITLEMENT-LEASE-1 → EDGE-ACTIVATION-1 | **next (gated)** |
