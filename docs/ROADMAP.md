...
| F3 | OFFLINE-EDGE-ENTITLEMENT-1 (module/add-on + gated UI/download/pairing/sync APIs + license/grace) | **next** |
| F3 | BRANCH-DEVICE-PAIRING-1 → BRANCH-BOOTSTRAP-SNAPSHOT-1 → EDGE-SALE-CAPTURE-1 → OFFLINE-SYNC-ENGINE-1 → EDGE-BUILD-STRIPPER-1 → EDGE-BUILD-PACKAGING-1 → LOCAL-PRINT-LAN-1 → SYNC-EXCEPTION-DASHBOARD-1 → MODE-RECONCILIATION-1 → pilot | queued |
