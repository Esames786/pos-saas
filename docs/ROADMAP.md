...
| F3 | ✅ **EDGE-LOCAL-RUNTIME-PREFLIGHT-1** (audit) — code-grounded map of turning the Edge foundation into a real disconnected Branch POS with minimum change: reuse map (most POS surface is class A/B; `finalizePaidSale` = the cloud-only GL/COGS/FEFO boundary to REPLACE with `EdgeSaleCaptureService`), local MariaDB + atomic staged bootstrap import, local auth via encrypted credential section (bootstrap omits passwords), POS dependency + first-offline matrices, identity/idempotency reuse (no double-post), inventory/cancellation/shift/sync/recovery/security boundaries, smallest new-component set, 7-sprint roadmap, release gates. Doc: `docs/audits/edge-local-runtime-preflight-2026-08.md` | ✅ design only |
| F3 | (next) **EDGE-LOCAL-RUNTIME-1** — local MariaDB + atomic bootstrap import + hard branch binding (no selling yet) → EDGE-LOCAL-AUTH-1 → EDGE-LOCAL-POS-1 → EDGE-LOCAL-PRINT-1 → OFFLINE-SYNC-ENGINE-1 → OFFLINE-RECOVERY-HARDEN-1 → EDGE-ACTIVATION-1 | **next** |
