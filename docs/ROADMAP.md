...
| F3 | (next, GATED) **EDGE-RUNTIME-BOUNDARY-1** — restricted build/route-allowlist skeleton (no cloud admin/finance/purchasing/mfg/billing; no DB/auth/selling) — safe to start ONLY after packaging/update/TLS contracts (§H2.14/§15/§H2.1) decided. In parallel sign off §H2.1 refresh / §H2.7 identity / §H2.8 sale_no / §H2.4 recipe-phase / §H2.5 activation-fence / §H2.9 envelope BEFORE any selling code → EDGE-LOCAL-RUNTIME-1 → EDGE-LOCAL-AUTH-1 → EDGE-IDENTITY-1 → EDGE-LOCAL-POS-1(phase-1a) → EDGE-LOCAL-PRINT-1 → EDGE-SPLITBRAIN-STOCK-1 → EDGE-CONFIG-REFRESH-1 → OFFLINE-SYNC-ENGINE-1 → OFFLINE-RECOVERY-HARDEN-1 → EDGE-ENTITLEMENT-LEASE-1/UPDATE-CHANNEL-1/OBSERVABILITY-1 → EDGE-RECIPE-OFFLINE-1(phase-1b) → EDGE-ACTIVATION-1. **Runtime coding still BLOCKED pending these design closures.** | **blocked (design closure)** |
